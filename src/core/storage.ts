...
export const STORAGE_FAILURE_EVENT = "ldu:storage-failure";
let storageFailureReported = false;

function reportStorageFailure(operation: string, key: string): void {
  if (storageFailureReported) return;
  storageFailureReported = true;
  console.warn(`[Linux.do Ultimate] 本地存储不可用，${operation} ${key} 失败。`);
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent(STORAGE_FAILURE_EVENT, { detail: { operation, key } }));
  }
}

export function hasStorageFailure(): boolean {
  return storageFailureReported;
}
...
interface RestorableSessionEntry {
...
export function stageSessionClose(storage: StorageAdapter, session: SessionState, closedAt = Date.now()): void {
  ...
  const entries = readRestorableSessions(storage)
    .filter((entry) => entry.session.sessionId !== session.sessionId);
  entries.push({ session, closedAt });
  writeRestorableSessions(storage, entries.sort((a, b) => a.closedAt - b.closedAt).slice(-8));
}
...
export function cleanupExpiredSessions(storage: StorageAdapter, now = Date.now()): void {
  const retainedIndex: SessionIndexEntry[] = [];
  for (const entry of readSessionIndex(storage)) {
    const expired = now - entry.updatedAt >= SESSION_RETENTION_MS;
    if (expired) {
  ...
  writeSessionIndex(storage, retainedIndex);
}
